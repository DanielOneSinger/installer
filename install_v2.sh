...
git clone https://github.com/DanielOneSinger/allora-model.git
cd allora-model || exit


# Prompt for wallet name and seed phrase
read -p "Enter your wallet name: " wallet_name
read -p "Enter your seed phrase: " seed_phrase
read -p "Enter your coingecko api-key: " api_key
...
echo "正在更新 utils.py..."
sed -i 's/\("x-cg-demo-api-key": *\)os\.environ\.get("") *$/\1"'"$api_key"'" # API key updated/' utils.py
echo "utils.py 更新完成。"

echo "正在更新 model.py..."
sed -i 's/\("x-cg-demo-api-key": *\)"" *# *API key updated/\1"'"$api_key"'" # API key updated/' model.py
echo "model.py 更新完成。"

# 显示文件内容
echo "utils.py 内容："
grep "x-cg-demo-api-key" utils.py

echo "model.py 内容："
grep "x-cg-demo-api-key" model.py
